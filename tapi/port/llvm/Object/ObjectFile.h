...
public:
  ...
  void getName(StringRef &ref) const;
  uint64_t getAddress() const;
  uint64_t getIndex() const;
  uint64_t getSize() const;
  void getContents(StringRef &ref) const;
...
inline void SectionRef::getName(StringRef &ref) const {
  ref = OwningObject->getSectionName(SectionPimpl).get();
}
...
inline void SectionRef::getContents(StringRef &ref) const {
  Expected<ArrayRef<uint8_t>> Res =
      OwningObject->getSectionContents(SectionPimpl);
  if (Res) {
    ref = StringRef(reinterpret_cast<const char *>(Res->data()), Res->size());
  }
}
