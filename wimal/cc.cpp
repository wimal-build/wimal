...
static inline void BearReport(
  ...
  sendto(
      fd, data.data(), data.size(), 0, (sockaddr *) &address, sizeof(address));
  close(fd);
}
...
void Cc::Run(const Context *context, std::vector<std::string> extraArgs) {
  ...
  switch (context->machine) {
    ...
    case Context::MACHINE_A64_IOS:
      ...
      break;
    case Context::MACHINE_X86_SIM:
    case Context::MACHINE_X64_SIM:
  ...
  switch (context->machine) {
    ...
    case Context::MACHINE_X64_ANDROID:
      ...
      // Use libgcc instead of libunwind for android.
      args.emplace_back("--rtlib=libgcc");
      break;
    default: {
      break;
    }
